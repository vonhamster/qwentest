...
class DatabaseManager {
    constructor(dbPath) {
        this.db = null;
        this.dbPath = dbPath || path_1.default.join(process.cwd(), 'data', 'app.db');
    }
    ...
    connect() {
        ...
        ActiveRecord_1.ActiveRecord.setDb(this.db);
    ...
    `);
      ...
      `).run('admin', 'admin@example.com', passwordHash);
        ...
        VALUES (?, ?, ?, 10)
      `).run('Welcome to Yii2 TypeScript Analog', 'This is a sample post demonstrating the framework capabilities. Built with TypeScript, Express, and SQLite.', admin.id);
            this.db.prepare(`
        INSERT INTO posts (title, content, author_id, status)
        VALUES (?, ?, ?, 10)
      `).run('Features Overview', 'This framework includes: ActiveRecord ORM, MVC architecture, REST API support, Authentication, View templates, and Bootstrap 5 integration.', admin.id);
